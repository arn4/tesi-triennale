...
\begin{defn}[Stato di Choi]
Consideriamo un canale quantistico \(\Phi: \LinSet(\Hilb_a) \to \LinSet(\Hilb_b)\).
Sia \( \{\ket{i}\}_{i=1,\dots,d} \) una base ortonormale di \(\Hilb_a\).
\end{defn}
...
\[\Lambda_{s\colon r} \left[\hat{\rho}^{(s)}\right] =
