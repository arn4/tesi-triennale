...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.73\textwidth}
			...
			\[\Phi_{t\colon s}\left[\hat{\rho}_s\right]=\hat{\rho}_t = \Tr_E\left(\hat{U}_{SE}(t,s)(\hat{\rho}_s\otimes\eta_s)\hat{U}^\dag_{SE}(t,s)\right)\]
		\end{column}
	    \begin{column}{0.22\textwidth}
	    	\begin{figure}
	    		\resizebox{\textwidth}{!}{\input{img/open-quantum.tikz}}
	    	\end{figure}
	    \end{column}
	\end{columns}
    \pause
...
\begin{columns}
	...
	\begin{column}{0.3\textwidth}
		\resizebox{\textwidth}{!}{\input{img/iCP.tikz}}
	\end{column}	
\end{columns}
...
\begin{block}{CP divisibilità operativa [oCP]}
	\begin{columns}
	    ...
	    \end{column}
        \begin{column}{0.5\textwidth}
        	\resizebox{\textwidth}{!}{\input{img/oCP.tikz}}
        \end{column}	
    \end{columns}
...
\begin{columns}
	\begin{column}{0.68\textwidth}
		...
		\begin{itemize}
			\item \structure{\(\lambda_0\) non invertibile:} iCP non è ben definito.
			\item \structure{\(\lambda_0\) invertibile:} oCP\(\implies\)iCP, ma il viceversa non è vero.
		\end{itemize}
	\pause
	...
	\begin{column}{0.30\textwidth}
		\begin{figure}
			\resizebox{\textwidth}{!}{\input{img/classi-divisibilita.tikz}}
		\end{figure}
	\end{column}
\end{columns}
\begin{columns}
...
\item \structure{Formalismo \emph{process tensor}} \cite{markovcondition}: generalizzazione della distribuzione di  probabilità.
\begin{itemize}
	\item markovianità quantistica tramite \emph{causal break}
	\item caratterizzazione dei processi oCP
\end{itemize}

\item \structure{\emph{Past-future correlation}} \cite{budini2018quantum}: definizione alternativa di markovianità quantistica, basata su una definizione classica alternativa
...
\printbibliography
\centerline{\resizebox{2.3cm}{!}{\input{img/orso.tikz}}}
